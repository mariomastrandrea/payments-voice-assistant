    ...
    public convenience init?(
        ...
        do {
            fileContent = try String(
                contentsOfFile: vocabularyPath,
                encoding: .utf8
            )
        }
        catch {
            log("error reading the tokenizer vocabulary file.\n\(error.localizedDescription)")
            return nil
        }
        
        var tokenIdsMappings = [String:Int32]()
        
        for (index, token) in fileContent.components(separatedBy: .newlines).enumerated() {
            tokenIdsMappings[token] = Int32(index)
        }
